...
def test_mini_training_loop_convergence():
    ...
    for step in range(100):  # Enough steps to see convergence
        ...
        logits = model(x).output  # [batch, seq, vocab_size]
...
def test_learning_rate_sensitivity():
    ...
    config = TransformerConfig(
        vocab_size=30, d_model=32, n_heads=4, n_layers=1, output_mode="vocab"
    )
    ...
    assert not all(
        abs(losses_list[0] - loss) < 1e-3 for loss in losses_list[1:]
...
def test_weight_decay_effect():
    ...
    config = TransformerConfig(vocab_size=50, d_model=32, n_heads=4, n_layers=1)
    ...
    optimizer_with_decay = torch.optim.Adam(
        model_with_decay.parameters(), lr=0.01, weight_decay=0.01
    )
    ...
    for _ in range(50):
        ...
        output_no_decay = model_no_decay(x).output
        ...
        output_with_decay = model_with_decay(x).output
