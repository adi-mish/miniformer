...
def test_dropout_train_vs_eval_determinism():
    ...
    assert not torch.allclose(
        output3, output4, atol=1e-6
    ), "Train mode should have stochastic dropout"


def test_dropout_consistency_across_layers():
    """Test that dropout is applied consistently across all layers."""
    config = TransformerConfig(vocab_size=100, d_model=64, n_heads=8, n_layers=3, dropout=0.3)
    model = Transformer(config)
    ...
    for i in range(1, len(outputs)):
        ...
        ), f"Eval pass {i} differs from pass 0"


def test_dropout_preserves_shape():
    """Test that dropout doesn't change tensor shapes."""
    config = TransformerConfig(
        vocab_size=50, d_model=48, n_heads=6, n_layers=2, dropout=0.8  # Very high dropout
    )
    model = Transformer(config)
...
def test_zero_dropout_equivalence():
    ...
    config = TransformerConfig(
        vocab_size=100, d_model=32, n_heads=4, n_layers=2, dropout=0.0  # No dropout
    )
    model = Transformer(config)
    ...
    assert torch.allclose(
        eval_output, train_output, atol=1e-6
    ), "Zero dropout should make train/eval equivalent"
